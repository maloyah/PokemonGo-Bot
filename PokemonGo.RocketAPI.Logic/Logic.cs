    ...
    public class Logic
    {
        ...
        private async Task fncPokeStop(Client client, FortData[] pokeStops, bool metros30)
        {
            ...
            foreach (var pokeStop in pokeStops)
            {
                ...
                if (_clientSettings.MaxWalkingRadiusInMeters != 0 && distanceFromStart > _clientSettings.MaxWalkingRadiusInMeters)
                {
                    Logger.ColoredConsoleWrite(ConsoleColor.Green, "You're outside of the defined max. walking radius. Walking back! 才不要咧");
        ...
        private async Task ExecuteCatchLurePokemons(FortData currentFortData)
        {
            ...
            do
            {
                ...
                caughtPokemonResponse = await _client.Encounter.CatchPokemon(encounterId, fortId, bestPokeball,hit, normalizedReticleSize,
                    spinModifier,
                    normalizedHitPos
                    );
