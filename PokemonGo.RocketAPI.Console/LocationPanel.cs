...
namespace PokemonGo.RocketAPI.Console
{
	...
	public partial class LocationPanel : UserControl
	{
        ...
        private void initViewOnly(int team, int level, long exp)
        {
            ...
            Globals.infoObservable.HandleNewHuntStats += InfoObservable_HandleNewHuntStats;
        }

        private void InfoObservable_HandleNewHuntStats(string value)
        {
            //25.1356587994025/121.505194266508;Psyduck;636089390338410548;2016/9/8 下午 01:43:53
            string[] aa = value.Split(new char[] { ';' });
            string[] gps = aa[0].Split(new char[] { '/' });
            double Latitude = double.Parse(gps[0]);
            double Longitude = double.Parse(gps[1]);
            GMarkerGoogle pokemonMarker;

            POGOProtos.Enums.PokemonId pokeid = (POGOProtos.Enums.PokemonId)Enum.Parse(typeof(POGOProtos.Enums.PokemonId), aa[1]);

            Bitmap pokebitMap = Pokemons.GetPokemonMediumImage(pokeid);
            if (pokebitMap != null)
            {
                var ImageSize = new System.Drawing.Size(pokebitMap.Width, pokebitMap.Height);
                pokemonMarker = new GMarkerGoogle(new PointLatLng(Latitude, Longitude), pokebitMap) { Offset = new System.Drawing.Point(-ImageSize.Width / 2, -ImageSize.Height / 2) };
                pokemonMarker.ToolTipText = "CP:" + aa[4];
                pokemonMarker.ToolTip.Font = new System.Drawing.Font("Arial", 12, System.Drawing.GraphicsUnit.Pixel);
                pokemonMarker.ToolTipMode = MarkerTooltipMode.OnMouseOver;
                _pokemonOverlay.Markers.Add(pokemonMarker);
            }
